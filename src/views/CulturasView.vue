<template>
  <PageLayout
    ...
    subtitle="Definição de parâmetros ideais de clima e tempo para cada espécie."
  >
    ...
    <section class="registration-container-estoque">
      ...
      <div class="inventory-split-view">
        <div class="seed-list-container">
          <div
            v-for="c in culturasFiltradas"
            :key="c.id"
            class="mini-card"
            :class="{ active: culturaSelecionada?.id === c.id }"
            @click="selecionar(c)"
          >
            ...
            <div class="mini-card-cultura">
              Temp: {{ c.temperatura_minima }}°C a {{ c.temperatura_maxima }}°C
            </div>
          </div>
        </div>
        ...
        <div class="seed-detail-panel">
          ...
          <div v-else-if="culturaSelecionada">
            <div class="detail-header">
              ...
              <button class="btn-icon-delete" @click="deletar(culturaSelecionada.id)">
            ...
            <div class="detail-grid">
              <div class="detail-item">
                ...
                ><span>{{ culturaSelecionada.tempo_medio_colheita }}</span>
              </div>
              <div class="detail-item">
                <label>Umidade Ideal</label><span>{{ culturaSelecionada.umidade_ideal }}%</span>
              </div>
              <div class="detail-item full-width qty-destaque">
                ...
                <span class="qty-value" style="color: var(--primary-dark)"
                  >{{ culturaSelecionada.temperatura_minima }}°C a
                  {{ culturaSelecionada.temperatura_maxima }}°C</span
                >
              </div>
            </div>
          </div>
...
const culturasFiltradas = computed(() =>
  culturas.value.filter((c) => c.nome_cultura.toLowerCase().includes(busca.value.toLowerCase())),
)
